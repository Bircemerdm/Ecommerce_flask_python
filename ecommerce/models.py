# veritabanı tablolarımızı , veritabanı işlemlerimiz temsil eder, veritaban ile ilgili fonksiyonlarımızı yazdığımız yer olucak.
...
# dataclass veritabanına eklencej tabloyu temsil ediyor.Bunun altında table name i oluşturacağız
@dataclass
class User(db.Model):
    __tablename__ = "user"
    ...
    def add_user(cls, username, email, password):
        user = cls(
            None, username, email, password, True
        )  # none koymalısın yoksa id kısmı olarak orası boş geçecek id=username gibi bir şey olucak bu karışıklık olmasın diye __init__ fonksiyonum çatışmasın birbiriyle
        db.session.add(user)
        db.session.commit()
